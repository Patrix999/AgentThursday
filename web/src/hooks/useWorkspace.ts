...
import { authHeaders, clearSecret, getActiveContextId, setActiveContextId } from "../auth/secret";
 ...
 * it differs from the client's localStorage cache (`agent-thursday.contextId`),
    ...
    async function poll(): Promise<{ reconciled: boolean }> {
      try {
        ...
        if (shouldReconcile) {
          ...
          window.dispatchEvent(new CustomEvent("agent-thursday:context:reconciled", {
