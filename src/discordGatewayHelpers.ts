 ...
 * keep both in sync until consolidation. an earlier revision §B endorses this temporary
...
export type DirectIngestPayload = {
  ...
  // the `isDmOverride: false` an earlier revision set. Emitting `chatType`
...
export function eventToDirectPayload(
  ...
  return {
    ...
    // DM-ness from `guildId == null` and undo the an earlier revision fix on
