...
//
//   — yaml → embedded manifest codegen.
  ...
  "skills": [
    {
      ...
      "prompt_segment": "当卡片要求\"交付 patch / test doc / smoke json / completion report\"且 verifier 需要可下载 artifact 时，优先调用 artifact.write，不要走 LocalDoc relay 或 Discord paste。\n必须字段：cardId、filename、type、sourceAgent、content。type 只能是 patch / test_doc / smoke_json / completion_report；directory_bundle v1 不支持。\n写之前自检 content：不放 secret（AKIA / ghp_ / sk- / Bearer / PRIVATE KEY 等），超过类型 size cap 时先拆分。\n调用成功后把 envelope.transport_uri 写进通知 / 测试文档，让 verifier 用 /api/artifact/<card_id>/<filename> 复核。",
      "capability_class": "callable_tool_ready",
      "source_ref": {
        "provider": "internal",
        "reference": "docs/tools/artifact.write.0.1.0.yaml"
...
const DIRECTED_VALIDATION_YAML: string = "# Directed Validation skillset v0.1.0\n#\n#   — gives a directed-validation agent ( in production)\n# a single controlled E2E smoke surface so it can validate live admin\n# endpoints without seeing `AGENT_THURSDAY_SHARED_SECRET`.\n#\n# Goal: close the validation harness gap surfaced by  —\n#  previously replied \"blocked: missing auth/tool access\" when\n# asked for real E2E validation, because no agent-callable surface\n# existed for admin endpoints. This skillset wraps the smallest\n# possible such surface (`admin.smoke`) with a closed-enum case list\n# (v1: sandbox-exec-printf only), structured evidence, and\n# adapter-side secret mediation.\n#\n# Scope constraints (mirror  spec):\n#   - no general curl / shell proxy; only allowlisted case_ids;\n#   - secret never crosses the agent boundary (adapter reads\n#     `env.AGENT_THURSDAY_SHARED_SECRET` and injects it on the synthesized\n#     loopback Request);\n#   - evidence redaction + 1024-char cap on stdout/stderr;\n#   - missing env binding → structured `blocked`, not 503/throw;\n#   - no production write expansion; admin.smoke targets a route\n#     whose admin contract is already \"harmless inspectable smoke\".\n\nid: directed-validation\nname: Directed Validation\ndescription: 让 directed-validation agent（生产中通常是）对 allowlisted harmless admin endpoint 做端到端 smoke，返回结构化 evidence；secret 不外泄，缺权限时回 blocked。\nversion: 0.1.0\npurpose: expose admin.smoke as the single callable E2E validation surface for directed-validation agents; no shell proxy, no secret exposure\n\ntools:\n  - admin.smoke\n\nskills:\n  - id: directed.validation.smoke\n    name: Run allowlisted admin smoke\n    description: 调用 admin.smoke({case_id}) 对单个 allowlisted harmless 路径做 E2E 真实验证，返回 http_status + response shape + byte-counted evidence\n    tier: 2\n    tools:\n      - admin.smoke\n    prompt_segment: |\n      被要求做 directed E2E validation 且需要触达真实 admin endpoint 时优先调用 admin.smoke({case_id})。\n      闭合 case_id enum（v0.1.0）：\n        - \"sandbox-exec-printf\"（）：POST /api/sandbox/exec, printf -e2e；返回 stdout/exit_code/redaction/truncation evidence。\n        - \"remember-ack-empty-fallback\"（）：in-process helper exercise，返回 final_reply / fallback_applied。\n        - \"context-active-inspect-smoke\" / \"context-lifecycle-noop-smoke\" / \"compaction-plan-dry-run-smoke\" / \"archive-inspect-smoke\"（）：AgentThursdayAgent stub @callable read-only / no-op 探针，返回结构 counts。\n        - \"cli-status-dashboard-shape-smoke\"（）：/cli/status dashboard composition slice 的安全探针，返回 6 个 top-level key 的 presence booleans + outbox/patch-apply kind 判别 + drift flag 白名单名 + version field presence。注意：探针不跑 sweepStaleDraftEnvelopes 也不返回 raw payload。\n      其它任意 case_id 会被 schema 拒绝，不要拼接。\n      返回 status==\"ok\" 时按 case 抄写关键 evidence（sandbox-exec-printf → http_status/response.exit_code/evidence.stdout_bytes；cli-status-dashboard-shape-smoke → http_status/response.drift_flag_names/response.latest_outbox_kind/response.patch_apply_outbox_kind）。\n      status==\"blocked\" 时按 blocked.reason 上报为 harness gap，不要假装通过。\n      绝对不要尝试用 admin.smoke 跑 commit / push / deploy / 写 repo 的命令——case_id 是闭合 enum，任何此类企图都会被 schema 拒绝。\n    capability_class: callable_tool_ready\n    source_ref:\n      provider: internal\n      reference: docs/tools/admin.smoke.0.1.0.yaml\n    evidence_requirements:\n      - status\n      - case_id\n      - http_status\n      - response\n      - evidence\n\nworkflow_patterns:\n  - name: smoke-then-report\n    when: 卡片要求对 admin/debug endpoint 做端到端真实验证\n    steps:\n      - 调用 admin.smoke 选择对应 case_id\n      - 把 status / http_status / response.exit_code / evidence.stdout_bytes 抄进验证 evidence\n      - status==\"blocked\" 时按 blocked.reason 上报为 harness gap，绝不伪装通过\n    completion_signal: status==\"ok\" 且 http_status==200 且 response.success==true\n\nreasoning_protocol:\n  principles:\n    - 不要假设自己能直接构造 admin auth header；secret 由 adapter 边界注入\n    - evidence 里 stdout / stderr 可能已截断；以 truncated_* 标志为准\n    - allowlist 之外的 case_id 不要尝试拼接；schema 会拒绝\n  anti_patterns:\n    - 不要把 admin.smoke 当成 sandbox_exec 的替代来跑任意命令\n    - 不要在 blocked 时改写为 ok；harness gap 才是有用的 evidence\n    - 不要把 case_id 当成 free-form prompt 拼接\n\nevidence_protocol:\n  protocol_version: \"0.1.0\"\n\nsafety_policy:\n  policy_version: \"0.1.0\"\n  path_allowlist: []\n  path_denylist:\n    - \"**\"\n  cross_repo_writes: denied\n\nobservability:\n  emit_events:\n    - skillset.directed-validation.load\n    - skillset.directed-validation.unload\n  inspect_surfaces:\n    - /api/inspect/skillset/directed-validation\n\npolicy:\n  surface_modes: [enable, readonly]\n  default_tier_cap: 2\n  load_priority: 25\n";
const DIRECTED_VALIDATION_MANIFEST = ({
  "id": "directed-validation",
  "name": "Directed Validation",
  "description": "让 directed-validation agent（生产中通常是）对 allowlisted harmless admin endpoint 做端到端 smoke，返回结构化 evidence；secret 不外泄，缺权限时回 blocked。",
  "version": "0.1.0",
  ...
  "skills": [
    {
      ...
      "prompt_segment": "被要求做 directed E2E validation 且需要触达真实 admin endpoint 时优先调用 admin.smoke({case_id})。\n闭合 case_id enum（v0.1.0）：\n  - \"sandbox-exec-printf\"（）：POST /api/sandbox/exec, printf -e2e；返回 stdout/exit_code/redaction/truncation evidence。\n  - \"remember-ack-empty-fallback\"（）：in-process helper exercise，返回 final_reply / fallback_applied。\n  - \"context-active-inspect-smoke\" / \"context-lifecycle-noop-smoke\" / \"compaction-plan-dry-run-smoke\" / \"archive-inspect-smoke\"（）：AgentThursdayAgent stub @callable read-only / no-op 探针，返回结构 counts。\n  - \"cli-status-dashboard-shape-smoke\"（）：/cli/status dashboard composition slice 的安全探针，返回 6 个 top-level key 的 presence booleans + outbox/patch-apply kind 判别 + drift flag 白名单名 + version field presence。注意：探针不跑 sweepStaleDraftEnvelopes 也不返回 raw payload。\n其它任意 case_id 会被 schema 拒绝，不要拼接。\n返回 status==\"ok\" 时按 case 抄写关键 evidence（sandbox-exec-printf → http_status/response.exit_code/evidence.stdout_bytes；cli-status-dashboard-shape-smoke → http_status/response.drift_flag_names/response.latest_outbox_kind/response.patch_apply_outbox_kind）。\nstatus==\"blocked\" 时按 blocked.reason 上报为 harness gap，不要假装通过。\n绝对不要尝试用 admin.smoke 跑 commit / push / deploy / 写 repo 的命令——case_id 是闭合 enum，任何此类企图都会被 schema 拒绝。",
      "capability_class": "callable_tool_ready",
      "source_ref": {
        "provider": "internal",
        "reference": "docs/tools/admin.smoke.0.1.0.yaml"
...
const EXTERNAL_PUBLISHING_YAML: string = "# External Publishing skillset v0.1.0\n#\n# Canonical manifest per ADR 2026-05-07 IM-1.\n#  introduced this skillset for the first external-network\n# callable proof (LocalDoc).  moves it from hand-written\n# `src/skillset/manifests.ts` to YAML-driven codegen so it stays in\n# lockstep with the source of truth.\n\nid: external-publishing\nname: External Publishing\ndescription: 通过外部 callable tool 把 agentthursday 产出转成可分享的非敏感链接；首个 fixture 是 LocalDoc convert_text。\nversion: 0.1.0\npurpose: publish markdown content via an external callable tool and return non-sensitive evidence\n\ntools:\n  - localdoc.convert_text\n\nskills:\n  - id: localdoc.publish.convert_text\n    name: Publish to LocalDoc\n    description: 把 markdown 内容上传到 LocalDoc，回传 id / url / markdownUrl / status\n    tier: 3\n    tools: [localdoc.convert_text]\n    prompt_segment: 发送内容到外部 publishing endpoint 前确认 content 已是终稿；返回的 url / markdownUrl 才能引用。\n    capability_class: callable_tool_ready\n    source_ref:\n      provider: localdoc\n      endpoint: https://localdoc.example.com/api/convert\n      env_binding: LOCALDOC_API_KEY\n    evidence_requirements:\n      - id\n      - url\n      - markdownUrl\n      - status\n\nworkflow_patterns:\n  - name: publish-and-cite\n    when: 需要把一段 markdown 内容外部化为可分享链接\n    steps:\n      - 确认 content 是终稿、无 secret\n      - 调用 localdoc.convert_text\n      - 记录返回的 url / markdownUrl 作为非敏感 evidence\n    completion_signal: evidence 中包含 status:ok 且 url 非空\n\nreasoning_protocol:\n  principles:\n    - secret 缺失时必须返回 blocked，不假成功\n    - \"evidence 只允许非敏感字段：id / url / markdownUrl / status\"\n  anti_patterns:\n    - 不要把 API key 写进 prompt、log 或 evidence\n\nevidence_protocol:\n  protocol_version: \"0.1.0\"\n\nsafety_policy:\n  policy_version: \"0.1.0\"\n  path_allowlist: []\n  path_denylist:\n    - \"**\"\n  cross_repo_writes: denied\n\nobservability:\n  emit_events:\n    - skillset.external-publishing.load\n    - skillset.external-publishing.unload\n  inspect_surfaces:\n    - /api/inspect/skillset/external-publishing\n\npolicy:\n  surface_modes: [enable]\n  default_tier_cap: 3\n  load_priority: 40\n";
const EXTERNAL_PUBLISHING_MANIFEST = ({
  "id": "external-publishing",
  "name": "External Publishing",
  "description": "通过外部 callable tool 把 agentthursday 产出转成可分享的非敏感链接；首个 fixture 是 LocalDoc convert_text。",
  "version": "0.1.0",
...
const RESEARCH_STUB_YAML: string = "# Research skillset stub v0.0.1\n#\n# Purpose: prove the 175 manifest schema is not software-dev-hardcoded.\n# Loader at fabric-startup time should auto-discover this file via\n# loadAllManifests() (ADR 2026-05-07 IM-1) and successfully parse it\n# with the same 175 schema validator that handles software-dev.\n#\n# Tool contracts for web.search / web.fetch / pdf.read are NOT yet\n# fleshed out in 176 §4 (which sampled software-dev tools only). They\n# will be drafted alongside a full Research skillset after ; the\n# 176 schema can describe them per §1 fields without modification.\n# The purpose of this stub is schema-coverage, not runtime ship.\n\nid: research-stub\nname: Research (stub)\ndescription: stub skillset for web research / citation； 用于 acid-test 175 schema 通用性\nversion: 0.0.1\npurpose: gather external information and produce citations\n\ndependencies:\n  - observability\n\ntools:\n  - web.search\n  - web.fetch\n  - pdf.read\n\nskills:\n  - id: web.lookup\n    name: Search Web\n    description: web.search 检索关键词，回 ranked results\n    tier: 2\n    tools: [web.search]\n    prompt_segment: |\n      检索时写明 query intent；不基于记忆 fabricate 来源。\n    input_contract:\n      type: object\n      required: [query]\n      properties:\n        query: { type: string }\n        max_results: { type: integer }\n    output_contract:\n      type: object\n      properties:\n        results: { type: array }\n\n  - id: web.read\n    name: Fetch Web Page\n    description: web.fetch 下载页面为可读文本\n    tier: 2\n    tools: [web.fetch]\n    prompt_segment: |\n      fetch 后保留 url 与 content_hash，下游 cite 时引用。\n    input_contract:\n      type: object\n      required: [url]\n      properties:\n        url: { type: string }\n    output_contract:\n      type: object\n      properties:\n        content: { type: string }\n        content_hash: { type: string }\n\n  - id: pdf.extract\n    name: Read PDF\n    description: pdf.read 解 PDF 文本\n    tier: 2\n    tools: [pdf.read]\n    prompt_segment: |\n      pdf 解析后保留来源 url / 文档名 / page range；不基于记忆引用具体段落。\n    input_contract:\n      type: object\n      required: [path_or_url]\n      properties:\n        path_or_url: { type: string }\n        page_range: { type: string }\n    output_contract:\n      type: object\n      properties:\n        text: { type: string }\n\nworkflow_patterns:\n  - name: search-read-cite\n    when: \"面对 research 类问题\"\n    steps:\n      - \"web.search 找候选来源\"\n      - \"web.fetch / pdf.read 读全文\"\n      - \"汇总观点并标注来源 URL / 文档名\"\n    completion_signal: \"回答含 ≥1 条带来源标注的引用\"\n\nreasoning_protocol:\n  principles:\n    - \"标明所有来源；不以记忆代替检索\"\n  anti_patterns:\n    - \"不要在没有 fetch/read 证据的情况下引用具体段落\"\n  verification_hooks:\n    - name: pre-cite-evidence-check\n      trigger: \"before any final reply that contains a citation\"\n      action: \"verify each cited source has a corresponding fetch/read in this envelope\"\n\nevidence_protocol:\n  protocol_version: \"0.1.0\"\n  required_envelope: [intent, execution, evidence]\n  per_tool_emit:\n    web.fetch:\n      emit_events: [tool.web.fetch.dispatch, tool.web.fetch.result]\n      required_evidence: [evidence.external_artifacts]\n    pdf.read:\n      emit_events: [tool.pdf.read.dispatch, tool.pdf.read.result]\n      required_evidence: [evidence.external_artifacts]\n    web.search:\n      emit_events: [tool.web.search.dispatch, tool.web.search.result]\n      required_evidence: [evidence.search_refs]\n\nsafety_policy:\n  policy_version: \"0.1.0\"\n  path_allowlist: []         # research stub 不写 repo\n  path_denylist:\n    - \"**\"                    # 全 deny，纯 read-only / 网络只读\n  cross_repo_writes: denied\n  # global hard ban (force_push, secret put|delete, .git/, .env*, secrets/, kanban.advance(verified))\n  # 由 177 fabric-level policy enforce，本 manifest 不重复。\n\nobservability:\n  emit_events:\n    - skillset.research-stub.load\n    - skillset.research-stub.unload\n    - skillset.research-stub.switch_in\n    - skillset.research-stub.switch_out\n  inspect_surfaces:\n    - /api/inspect/skillset/research-stub\n\npolicy:\n  surface_modes: [enable, readonly]\n  default_tier_cap: 2\n  load_priority: 50\n";
const RESEARCH_STUB_MANIFEST = ({
  "id": "research-stub",
  "name": "Research (stub)",
  "description": "stub skillset for web research / citation； 用于 acid-test 175 schema 通用性",
  "version": "0.0.1",
  ...
  "skills": [
    {
      ...
      "prompt_segment": "检索时写明 query intent；不基于记忆 fabricate 来源。",
      "input_contract": {
        "type": "object",
        "required": [
          "query"
    ...
    },
    {
      ...
      "prompt_segment": "fetch 后保留 url 与 content_hash，下游 cite 时引用。",
      "input_contract": {
        "type": "object",
        "required": [
          "url"
    ...
    },
    {
      ...
      "prompt_segment": "pdf 解析后保留来源 url / 文档名 / page range；不基于记忆引用具体段落。",
      "input_contract": {
        "type": "object",
        "required": [
          "path_or_url"
  ...
  "evidence_protocol": {
    ...
    "per_tool_emit": {
      "web.fetch": {
        "emit_events": [
          "tool.web.fetch.dispatch",
          "tool.web.fetch.result"
        ],
        "required_evidence": [
          "evidence.external_artifacts"
        ]
      },
      "pdf.read": {
        "emit_events": [
          "tool.pdf.read.dispatch",
          "tool.pdf.read.result"
        ],
        "required_evidence": [
          "evidence.external_artifacts"
        ]
      ...
      "web.search": {
        "emit_events": [
          "tool.web.search.dispatch",
          "tool.web.search.result"
        ],
        "required_evidence": [
          "evidence.search_refs"
        ]
      }
    }
  },
...
const RUNTIME_INSPECTOR_BASIC_YAML: string = "# Runtime Inspector Basic skillset v0.1.0\n#\n#   — first skillset that references a YAML-sourced tool\n# contract (`skillset.runtime_summary`) and binds it as\n# `callable_tool_ready` through the generic agent dynamic tool mapper.\n#\n# Goal: prove a new skillset + new tool can ship end-to-end without\n# touching the loader, the contract registry's hand-written CONTRACTS\n# array, or AgentThursdayAgent.getTools(). The skillset declares a single\n# safe self-inspection skill so the agent can return a non-sensitive\n# runtime summary on demand.\n\nid: runtime-inspector-basic\nname: Runtime Inspector Basic\ndescription: 让 agent 在或操作员要求时返回一份非敏感的 skillset runtime 概览（loaded/rejected ids、tool surface、SOUL budget）。\nversion: 0.1.0\npurpose: expose skillset.runtime_summary as a callable self-inspection tool, with no secret / network / write side effects\n\ntools:\n  - skillset.runtime_summary\n\nskills:\n  - id: runtime.inspect.summary\n    name: Runtime summary\n    description: 调用 skillset.runtime_summary 返回当前加载/拒绝的 skillset id、暴露的 tool surface、SOUL token 估算\n    tier: 2\n    tools:\n      - skillset.runtime_summary\n    prompt_segment: 当被问到\"现在加载了哪些 skillset / 哪些 tool 可调用 / SOUL 还剩多少\"时优先调用 skillset.runtime_summary，不要凭直觉回答；返回 evidence 必须包含 schema_version / skillset_ids / tool_ids / status。\n    capability_class: callable_tool_ready\n    source_ref:\n      provider: internal\n      reference: docs/tools/skillset.runtime_summary.0.1.0.yaml\n    evidence_requirements:\n      - schema_version\n      - skillset_ids\n      - tool_ids\n      - status\n\nworkflow_patterns:\n  - name: self-inspect-on-request\n    when: 操作员 /  询问 agent 当前 skillset 或 tool 装配状态\n    steps:\n      - 调用 skillset.runtime_summary\n      - 把返回的 loaded / rejected / tool_ids 转写成简短摘要\n      - 不要泄露任何 env 值 / 任何 disable 记录的 reason\n    completion_signal: evidence.status === \"ok\" 且 skillset_ids.loaded 非空\n\nreasoning_protocol:\n  principles:\n    - 自检工具不能替代真实的事故响应；仍要遵守 escalation\n    - 返回的字段已经是非敏感子集；不要再额外 prefix env binding\n  anti_patterns:\n    - 不要在没调 tool 的情况下凭记忆报 skillset 数量\n    - 不要把 source_shas 或 disable reason 加进 evidence\n\nevidence_protocol:\n  protocol_version: \"0.1.0\"\n\nsafety_policy:\n  policy_version: \"0.1.0\"\n  path_allowlist: []\n  path_denylist:\n    - \"**\"\n  cross_repo_writes: denied\n\nobservability:\n  emit_events:\n    - skillset.runtime-inspector-basic.load\n    - skillset.runtime-inspector-basic.unload\n  inspect_surfaces:\n    - /api/inspect/skillset/runtime-inspector-basic\n\npolicy:\n  surface_modes: [enable, readonly]\n  default_tier_cap: 2\n  load_priority: 30\n";
const RUNTIME_INSPECTOR_BASIC_MANIFEST = ({
  "id": "runtime-inspector-basic",
  "name": "Runtime Inspector Basic",
  "description": "让 agent 在或操作员要求时返回一份非敏感的 skillset runtime 概览（loaded/rejected ids、tool surface、SOUL budget）。",
  "version": "0.1.0",
...
const SOFTWARE_DEV_MANIFEST = ({
  ...
  "skills": [
    ...
    },
    {
      ...
      "prompt_segment": "在把 patch artifact 交给 之前，先用 patch.validate 跑一遍：hunk-count\naudit + git apply --check + 隔离 apply + 新文件 EOF 校验，可选 node --check\nlightweight gate。输入用 artifact.write 后的 cardId + filename 引用，或\n直接传 patchText（前者带 sha256 evidence，优先用 ref）。\nfailureReason='hunk_count_mismatch' 是 243 类缺陷的强信号，不要重交；先\n重新生成 patch 再 validate。这条 surface 只读写 ephemeral /tmp 沙盒，不\n会触 commit/push/deploy。",
      "capability_class": "callable_tool_ready",
      "source_ref": {
        "provider": "internal",
        "reference": "docs/tools/patch.validate.0.1.0.yaml"
...
export const EMBEDDED_MANIFESTS: EmbeddedManifest[] = [
  ...
  { id: "directed-validation", source_yaml: DIRECTED_VALIDATION_YAML, manifest: DIRECTED_VALIDATION_MANIFEST },
  { id: "external-publishing", source_yaml: EXTERNAL_PUBLISHING_YAML, manifest: EXTERNAL_PUBLISHING_MANIFEST },
  ...
  { id: "research-stub", source_yaml: RESEARCH_STUB_YAML, manifest: RESEARCH_STUB_MANIFEST },
  { id: "runtime-inspector-basic", source_yaml: RUNTIME_INSPECTOR_BASIC_YAML, manifest: RUNTIME_INSPECTOR_BASIC_MANIFEST },
