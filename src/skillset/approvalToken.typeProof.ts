/**
 *  Type — type-level proof that the current evidence envelope
 * accepts `evidence.approval_decision` as a closed-set member.
 *
 *  ADR Type asks: does the existing envelope schema admit an
 * `approval_decision` evidence entry, or do we need a separate
